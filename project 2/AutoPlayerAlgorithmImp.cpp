	...
	for (int i = 0; i < NUM_OF_ROWS; ++i) {
		for (int j = 0; j < NUM_OF_COLS; ++j) {
			...
			if (owningPlayer == opponentNumber) {
				...
				if(myGameBoard.getBoard()[x - 1][y - 1].getJokerRep() == '#')
...
unique_ptr<Move> AutoPlayerAlgorithmImp::getMove(){
	...
	if (coin <= 4) {
		...
		while (myGameBoard.getBoard()[fromX-1][fromY-1].getPiece() == 'F' || myGameBoard.getBoard()[fromX-1][fromY-1].getPiece() == 'f'
			|| myGameBoard.getBoard()[fromX-1][fromY-1].getPiece() == 'B' || myGameBoard.getBoard()[fromX-1][fromY-1].getPiece() == 'b') {
		...
		if (!doOpponentOccupiePoint(toX, toY)) {
			...
			myGameBoard.setCell(toX, toY, myPlayerNumber, myGameBoard.getBoard()[fromX][fromY].getPiece(), myGameBoard.getBoard()[fromX][fromY].getJokerRep());
		...
		//fight gonna happen
			...
			auxPiece.setPiece(myGameBoard.getBoard()[fromX-1][fromY-1].getPiece());
			auxPiece.setOwner(myPlayerNumber);
			auxPiece.setJokerRep(myGameBoard.getBoard()[fromX-1][fromY-1].getJokerRep());
	...
	//move to cell Occupied by oponnent 
		...
		for (int i = 0; i < oponnentSize; ++i) {
			if ((toolThatOponnentAte = myGameBoard.getBoard()[opponentOccupiedPoints[i].getX() - 1][opponentOccupiedPoints[i].getY() - 1].getJokerRep()) != '#') {
				for (int j = 0; j < mySize; ++j) {
					if (myGameBoard.getBoard()[myOccupiedPoints[j].getX() - 1][myOccupiedPoints[j].getY() - 1].getPiece() != toolThatOponnentAte
						&& myGameBoard.getBoard()[myOccupiedPoints[j].getX() - 1][myOccupiedPoints[j].getY() - 1].getPiece() != 'F'
						&& myGameBoard.getBoard()[myOccupiedPoints[j].getX() - 1][myOccupiedPoints[j].getY() - 1].getPiece() != 'f'
						&& myGameBoard.getBoard()[myOccupiedPoints[j].getX() - 1][myOccupiedPoints[j].getY() - 1].getPiece() != 'B'
						&& myGameBoard.getBoard()[myOccupiedPoints[j].getX() - 1][myOccupiedPoints[j].getY() - 1].getPiece() != 'b'
						&& myGameBoard.getBoard()[myOccupiedPoints[j].getX() - 1][myOccupiedPoints[j].getY() - 1].getPiece() != 'J' 
						&& myGameBoard.getBoard()[myOccupiedPoints[j].getX() - 1][myOccupiedPoints[j].getY() - 1].getPiece() != 'j') {
			...
			if ((toolThatOponnentAte = myGameBoard.getBoard()[opponentOccupiedPoints[i].getX() - 1][opponentOccupiedPoints[i].getY() - 1].getJokerRep()) != '#') {
				for (int j = 0; j < mySize; ++j) {
					if (myGameBoard.getBoard()[myOccupiedPoints[j].getX() - 1][myOccupiedPoints[j].getY() - 1].getPiece() != toolThatOponnentAte
						...
						JokerChangeImp joker(dest, jokerCh);
						(*jokerChange) = joker;
...
unique_ptr<JokerChange> AutoPlayerAlgorithmImp::getJokerChange() {
	if (acuredJokerChange) {
		unique_ptr<JokerChange> j = make_unique<JokerChangeImp>(*jokerChange);
		return j;
	}
	return nullptr;
}